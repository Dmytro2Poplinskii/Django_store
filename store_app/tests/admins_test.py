...
class AdminTest(TestCase):
    ...
    def test_product_admin(self):
        ...
        product = Product.objects.create(title="Test Product", selling_price=10.00, discount_price=5.00, category="CR")

        response = self.client.get(reverse('admin:store_app_product_changelist'))
        self.assertContains(response, "Test Product")

        response = self.client.get(reverse('admin:store_app_product_change', args=[product.id]))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Test Product")
